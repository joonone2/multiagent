...
가장 기본적인 형태로, 정해진 순서대로 에이전트들이 발언합니다.

* **흐름:** `Debater` ➡ `Verifier` ➡ `Moderator` (무한 반복)
* **특징:**
* 대화 히스토리가 자동으로 공유됨.
* `Verifier`가 승인하거나 `Moderator`가 "최종 답변"을 선언하면 종료 조건(`Termination`)에 의해 멈춤.
...
LLM(지능)이 대화 맥락을 파악하여 **"다음에 누가 말할지"** 를 스스로 결정합니다.
...
복잡한 업무 절차를 **그래프(설계도)** 로 그려서 강제하는 방식입니다.
...
| **GraphFlow** | Edge Map | 설계된 화살표 따라 이동 | **복잡한 SOP, 병렬 처리** |
